...
//
// MatrixMedia FORK NOTE (2026-05-07): TrackType is moved BEFORE
// CodecID and CodecPrivate so the EBML output emits TrackType (0x83)
// first among the track's child elements. ExoPlayer's
// MatroskaExtractor (and other strict Matroska parsers) finalize the
// codec mapping at TrackType time — if CodecPrivate arrives earlier
// the parser has nowhere to stash the OpusHead bytes and reports
//   "Missing CodecPrivate for codec A_OPUS{contentIsMalformed=true}".
// ebml-go marshals struct fields in struct-definition order via
// reflect.Field(i), so the field POSITION here directly controls the
// output element order — not the struct-literal position at the call
// site. Upstream order is preserved for every other field.
