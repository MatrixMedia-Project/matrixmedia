...
impl Database for SqliteDatabase {
    ...
    async fn bump_stream_marker_generation(
        ...
        sqlx::query("UPDATE mm_streams SET marker_generation = marker_generation + 1 WHERE id = ?1")
            .bind(&stream_id.0)
            .execute(&self.pool)
            .await
            .map_err(db_err)?;
        let generation: i64 =
            sqlx::query_scalar("SELECT marker_generation FROM mm_streams WHERE id = ?1")
                .bind(&stream_id.0)
                .fetch_one(&self.pool)
                .await
                .map_err(db_err)?;
