...
func TestBlockWriter_NewSimpleWriter(t *testing.T) {
	...
	expectedBytes := []byte{
		...
		0xD7, 0x81, 0x01,
		0x73, 0xC5, 0x81, 0x02,
		// MatrixMedia FORK NOTE: TrackType (0x83) is emitted BEFORE
		// CodecID (0x86) — see the TrackEntry struct reorder in webm.go.
		// Upstream expected 0x86,0x80 then 0x83,0x81,0x01.
		0x83, 0x81, 0x01,
		0x86, 0x80,
